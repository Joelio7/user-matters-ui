...
export const fetchCustomerMatters = createAsyncThunk(
  ...
  async (customerId: number, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.message || 'Failed to fetch customer matters');
    }
  }
);
...
export const createCustomerMatter = createAsyncThunk(
  ...
  async ({ customerId, data }: { customerId: number; data: MatterFormData }, { rejectWithValue }) => {
    try {
...
export const updateMatter = createAsyncThunk(
  'matters/update',
  async ({ id, data }: { id: number; data: Partial<MatterFormData> }, { rejectWithValue }) => {
    try {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateMatter.fulfilled, (state, action) => {
        const index = state.matters.findIndex(m => m.id === action.payload.id);
        if (index !== -1) {
          state.matters[index] = action.payload;
        }
        const customerIndex = state.customerMatters.findIndex(m => m.id === action.payload.id);
        if (customerIndex !== -1) {
      ...
      .addCase(deleteMatter.fulfilled, (state, action) => {
        ...
        state.customerMatters = state.customerMatters.filter(m => m.id !== action.payload);
